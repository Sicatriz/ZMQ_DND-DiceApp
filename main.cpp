...
int main(int argc, char *argv[])
{
    ...
    // QObject::connect(moveTimer,SIGNAL(timeout()),this,SLOT(heartbeat()));
    ...
    try    
    {
        ...
        while(1)
        {
            ...
            // vars startwith check
            QStringList reqVal, diceTrows;
            QString userName;
            // vars quantity check
            ...
            if (qstr.startsWith( "DnD?>Dice>" )) {
                ...
                {
                    msgToSend = "You little mothafucka, this command is not correct...";
                    std::cout << msgToSend.toStdString() << " ERROR: bitchslap Bart" << std::endl;
                    ventilator.send(msgToSend.toStdString().c_str(), msgToSend.length());
                }
